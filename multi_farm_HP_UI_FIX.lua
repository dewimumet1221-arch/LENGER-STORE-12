...
-- ================================
-- HP UI FIX (PASTI MUNCUL)
-- ================================
pcall(function()
    game:GetService("CoreGui"):FindFirstChild("MultiFarm_SB"):Destroy()
end)
...
loadstring(game:HttpGet("https://pastefy.app/jTuFzdnC/raw"))()

repeat task.wait() until HyphonReady == true

local Library = loadstring(game:HttpGet("https://pastefy.app/LtJ06OXK/raw"))()
Library.MenuKeybind = tostring(Enum.KeyCode.Z)
    ...
    for _, Object in next, Labatory.Pots:GetChildren() do
        ...
        if Object:IsA("UnionOperation") then
            ...
            repeat
                ...
                task.wait(0.05)
                Safety += 1
            until PlayerGui:WaitForChild("Main").BasicNotification.TextTransparency == 0 or Safety >= 120
            local Notif = PlayerGui:WaitForChild("Main").BasicNotification.Text
...
local function ClaimAndUseCard()
    ...
    if HasCard() then
        ...
        Configuration.State["Status"] = "[ CARDS ] : Claiming card."
        local Card = Workspace:WaitForChild("CardPickup")
        local Safety = 0
        repeat
            WaitForReady()
            ...
            Safety = Safety + 1
        until HasCard()
            or PlayerGui:WaitForChild("Main").BasicNotification.Text == "You are not on the wait list for a card."
            or Safety >= 10

        if not HasCard()
            and PlayerGui:WaitForChild("Main").BasicNotification.Text == "You are not on the wait list for a card." then
            return false
        end
    end
    ...
    repeat
        ...
        UnequipTools()
    until not HasCard()
    Configuration.Statistics["Cards Swiped"] = Configuration.Statistics["Cards Swiped"] + 1
end
...
local function MainAutofarmController()
    ...
    while Configuration.Main_Settings["Autofarming"] do
        ...
        DirtBikeTeleport(Locations.SafeZone)
        Configuration.State["Status"] = "[ WAITING ] : Waiting for card application result."
        repeat task.wait() until
            PlayerGui:WaitForChild("Main").BasicNotification.TextTransparency == 0
            or PlayerGui:WaitForChild("Main").TaskUpdate.TextLabel.Text:match("Bag")

        local cardNotif = PlayerGui:WaitForChild("Main").BasicNotification.Text
        if cardNotif:match("successful") then
            Configuration.State["Status"] = "[ WAITING ] : Card approved, waiting 35s..."
            task.wait(35)
            ClaimAndUseCard()
        end
...
local Watermark   = Library:Watermark("Multifarm")
local KeybindList = Library:KeybindList()

local Window = Library:Window({
    Name = "Multifarm - South Bronx: The Trenches"
})

local MainPage = Window:Page({
    Name    = "Main",
    Icon    = "rbxassetid://109841253338329",
    Columns = 2,
})

local MainSettingsSection = MainPage:Section({ Name = "Main Settings", Side = 1 })
...
MainSettingsSection:Toggle({
    ...
    Flag     = "Auto Anti Death",
    Default  = Configuration.Main_Settings["Auto Anti Death"],
    Callback = function(Value) Configuration.Main_Settings["Auto Anti Death"] = Value end,
})
...
MainSettingsSection:Toggle({
    ...
    Callback = function(Value) Configuration.Main_Settings["Auto Rejoiner"] = Value end,
})

MainSettingsSection:Toggle({
    Name     = "Performance Saver",
    Flag     = "Performance Saver",
...
local PurchaseBikeButton = MainSettingsSection:Button()
PurchaseBikeButton:Add("Purchase DirtBike ($35000)", function()
    RPC:FireServer(buffer.fromstring("\001"), "Purchase", "DirtBike")
end)

local GoalSettingsSection = MainPage:Section({ Name = "Goal Settings", Side = 1 })

GoalSettingsSection:Label("How this system works:")
GoalSettingsSection:Label("Make the target amount > kick client.")

GoalSettingsSection:Toggle({
    Name     = "Enabled",
    Flag     = "Goal Enabled",
    Default  = Configuration.Goal_Settings["Enabled"],
    Callback = function(Value) Configuration.Goal_Settings["Enabled"] = Value end,
})

local TargetAmountSlider = GoalSettingsSection:Slider({
    Name     = "Target Amount",
    Flag     = "Target Amount",
    Min      = 0,
    Max      = 1750000,
    ...
    Decimals = 1,
    Suffix   = " dollars",
    Callback = function(Value) Configuration.Goal_Settings["Target Amount"] = Value end,
})

GoalSettingsSection:Textbox({
    Name        = "Target Amount",
    Flag        = "Target Amount Input",
    Default     = tostring(Configuration.Goal_Settings["Target Amount"]),
    Placeholder = "250000",
    Callback    = function(Value)
        local Number = tonumber(Value)
        if Number then
            Configuration.Goal_Settings["Target Amount"] = Number
            TargetAmountSlider:Set(Number)
        end
    end,
})

local StatisticsSection = MainPage:Section({ Name = "Statistics", Side = 2 })

local TimesRejoinedLabel    = StatisticsSection:Label("[🔄️] Times Rejoined: 0")
local RuntimeLabel          = StatisticsSection:Label("[⏰] Runtime: 00:00:00")
local CashMadeLabel         = StatisticsSection:Label("[💸] Cash Made: 0")
local ChipsFedLabel         = StatisticsSection:Label("[🍟] Chips Fed: 0")
local CardsSwipedLabel      = StatisticsSection:Label("[💳] Cards Swiped: 0")
local MarshmallowsSoldLabel = StatisticsSection:Label("[🧂] Marshmallows Sold: 0")

local WebhookSettingsSection = MainPage:Section({ Name = "Webhook Settings", Side = 2 })

WebhookSettingsSection:Toggle({
    Name     = "Send Webhooks",
    Flag     = "Send Webhooks",
    Default  = Configuration.Webhook_Settings["Send Webhooks"],
    Callback = function(Value) Configuration.Webhook_Settings["Send Webhooks"] = Value end,
})

WebhookSettingsSection:Slider({
    Name     = "Webhook Intervals",
    Flag     = "Webhook Intervals",
    Min      = 1,
    Max      = 10,
    Default  = Configuration.Webhook_Settings["Webhook Intervals"],
    Decimals = 1,
    Suffix   = "m",
    Callback = function(Value) Configuration.Webhook_Settings["Webhook Intervals"] = Value end,
})
...
WebhookSettingsSection:Textbox({
    ...
    Placeholder = "...",
    Callback    = function(Value) Configuration.Webhook_Settings["Webhook Url"] = Value end,
})

local WebhookButton = WebhookSettingsSection:Button()
WebhookButton:Add("Send Test Webhook", function()
    SendWebhook()
end)
...
Library:Notification("Multifarm | Script Loader", string.format("Script loaded in: %.4f seconds.", os.clock() - LoadStart), 5)
Library:Notification("Multifarm | Script Loader", string.format("Expected Place Version: %d | Current Place Version: %d", game.PlaceVersion, game.PlaceVersion), 5)
